...
O Lithe possui um middleware de sessão para gerenciar as sessões dos usuários e suas variáveis. Para configurar o middleware em sua aplicação, adicione-o com o método `use()`:
...
- **lifetime**: Define o tempo de vida da sessão em segundos. O padrão é 2592000 segundos (30 dias).
- **domain**: Define o domínio para o qual o cookie da sessão é válido. O padrão é `null`.
- **secure**: Indica se o cookie da sessão deve ser enviado apenas por conexões seguras (HTTPS). O padrão é `false`.
...
- **get(string $name, mixed $default = null): mixed**
  ...
  Obtém o valor de uma variável de sessão. Retorna o valor padrão se a variável não estiver definida.
...
- **regenerate(bool $deleteOldSession = true): bool**
  ...
  Regenera o ID da sessão. Se `$deleteOldSession` for `true`, a antiga sessão será deletada.
...
- **Criação do Diretório de Sessão**: O middleware garante que o diretório para armazenamento das sessões exista. Se não existir, ele será criado.
- **Tratamento de Erros**: Caso ocorra um erro durante a configuração ou o início da sessão, o middleware irá registrá-lo e continuar a execução.