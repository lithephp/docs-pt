...
Você pode usar o comando Line `make:migration` para gerar uma migração de banco de dados. A nova migração será colocada no diretório `database/migrations` do seu projeto. Cada nome de arquivo de migração contém um timestamp que permite ao Lithe determinar a ordem das migrações.
