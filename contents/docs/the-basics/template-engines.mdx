...
Defina o diretório onde os arquivos de modelo estão localizados. 
...
Configure o mecanismo de template a ser usado pelo Lithe.

```php
$app->set('view engine', 'blade');
```

O Lithe é compatível com vários motores de template populares, como Blade, Twig e PHP puro. Por padrão, o Lithe utiliza PHP puro.

### 3. Instalação do Pacote

Após configurar o motor de template, instale o pacote Composer correspondente ao mecanismo escolhido para garantir que tudo funcione corretamente.
...
Use o método `engine` da classe `Lithe\App` para configurar um motor ou criar seu próprio motor de template. 
