...
Para gerar rapidamente um novo controlador, você pode executar o comando Line `make:controller`. Por padrão, todos os controladores da sua aplicação gerados pelo comando são armazenados no diretório `http/controllers`:
