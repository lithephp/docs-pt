...
Você também pode especificar a interface `Lithe\Contracts\Http\Request` como tipo no fechamento de uma rota.
...
Entendi, vou ajustar o guia do `query` para seguir o mesmo formato do guia de `cookies`. Aqui está a versão atualizada:
