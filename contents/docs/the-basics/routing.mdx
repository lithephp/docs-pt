...
#### 1. Criação do Arquivo do Roteador
...
#### 2. Integração do Roteador na Aplicação Principal

Agora, você precisa carregar o módulo do roteador na aplicação principal. Abra o arquivo `App.php` e adicione o seguinte código:

```php:src/App.php
$birds = require(__DIR__ . '/birds.php'); // Carrega o roteador de pássaros
...
Outra maneira de estruturar seu roteador é usando uma abordagem funcional, que pode ser mais intuitiva em algumas situações.
...
Crie o arquivo `birds.php`, mas desta vez usando uma sintaxe funcional:
...
Como antes, você precisa carregar o roteador na aplicação:

```php:src/App.php
use function Lithe\Orbis\Http\Router\{router};

$birds = router(__DIR__ . '/birds'); // Carrega o roteador

// Monta o roteador na aplicação
$app->use('/birds', $birds);
```

Agora, sua aplicação estará pronta para lidar com requisições para `/birds` e `/birds/sobre`, além de invocar o middleware `$timeLog`, que é específico para o roteador.