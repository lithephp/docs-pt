...
No Lithe, você define como seu aplicativo responde a solicitações dos clientes para endpoints específicos usando URIs (caminhos) e métodos HTTP como GET, POST, entre outros. Métodos como `$app->get()` são usados para lidar com solicitações GET, `$app->post()` para POST, e `$app->use()` para aplicar middleware, que são funções de callback para manipulação adicional das requisições.

Esses métodos especificam funções de callback, também chamadas de "manipuladores", que são acionadas quando o aplicativo recebe uma solicitação para a rota e método HTTP definidos. Cada rota no Lithe pode ter uma ou mais funções de manipulador, que são executadas quando a rota é correspondida. É importante utilizar `$next()` para passar o controle para a próxima função de callback, especialmente quando há várias funções definidas.
...
Crie um arquivo de roteador chamado `birds.php` no diretório da aplicação (`src`), com o seguinte conteúdo:

```php:src/birds.php showLineNumbers
<?php 
